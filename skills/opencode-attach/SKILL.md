...
description: "Delegate ALL coding tasks to OpenCode, a dedicated AI coding agent running as a headless server. Use when you need to write code, debug, refactor, scaffold projects, review code, run tests, or modify source files. Do not write code directly — delegate to OpenCode via its CLI. Triggers on any request involving code generation, file creation, debugging, refactoring, code review, test execution, or project scaffolding."
---

# OpenCode Coding Agent

You have access to OpenCode, a dedicated AI coding agent running as a headless server.
...
The server address and authentication credentials must be available in your environment.

## Quick Reference

| Task | Command |
|------|---------|
| New coding task | `opencode run --attach <server-url> "task description"` |
| Continue last session | `opencode run --attach <server-url> --continue "follow-up"` |
| Continue specific session | `opencode run --attach <server-url> --session SESSION_ID "follow-up"` |
...
```bash
opencode run --attach <server-url> "Implement a REST API endpoint for user authentication"
```

Runs a single prompt against the OpenCode server. Creates a new session.
All permissions are auto-approved in non-interactive mode.

### Give the session a title

```bash
opencode run --attach <server-url> --title "User Auth API" "Scaffold a FastAPI project with JWT auth"
```
...
```bash
opencode run --attach <server-url> --continue "Now add input validation"
```

Picks up the last session and continues where it left off.

### Continue a specific session

```bash
opencode run --attach <server-url> --session ses_abc123 "Fix the failing test"
```
...
```bash
opencode run --attach <server-url> --session ses_abc123 --fork "Try an alternative approach"
```
...
```bash
opencode run --attach <server-url> --model anthropic/claude-opus-4-6 "Complex refactor task"
```
...
```bash
opencode run --attach <server-url> --agent coder "Write the implementation"
```
...
```bash
opencode run --attach <server-url> --file error_log.txt "Debug this error"
```
...
```bash
opencode run --attach <server-url> --format json "List all TODO comments in the codebase"
```
...
```bash
opencode run --attach <server-url> -q "Quick task"
```
...
```bash
opencode run --attach <server-url> "Create a Python script that reads CSV and outputs JSON"
```
...
opencode run --attach <server-url> --title "User Auth API" \
...
opencode run --attach <server-url> --continue \
...
```bash
# Original approach
opencode run --attach <server-url> --session ses_abc123 \
  "Implement caching with Redis"

# Fork and try a different approach
opencode run --attach <server-url> --session ses_abc123 --fork \
  "Implement caching with in-memory LRU cache instead"
```

## Important Notes

- **Sessions persist.** Every coding interaction is saved and can be reviewed later.
